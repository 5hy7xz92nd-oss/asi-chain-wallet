...
const updateSelectedAccountForNetwork = (state: WalletState) => {
    ...
    if (typeof window !== "undefined" && window.localStorage) {
        ...
        if (storedId) {
            nextSelected =
                availableAccounts.find((acc) => acc.id === storedId) || null;
        }
    }
...
const loadPendingTransactions = (): PendingTransaction[] => {
    ...
    try {
        ...
        console.error(
            "Failed to load pending transactions from localStorage:",
            error,
        );
        return [];
    }
...
const removePendingTransaction = (deployId: string) => {
    ...
    try {
        ...
        console.error(
            "Failed to remove pending transaction from localStorage:",
            error,
        );
    }
};
...
const calculateBalanceWithPending = (
    baseBalance: string,
    accountId: string,
    revAddress: string,
    publicKey: string,
...
const walletSlice = createSlice({
    ...
    reducers: {
        ...
        updateAccountBalance: (
            state,
            action: PayloadAction<{ accountId: string; balance: string }>,
        ) => {
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(fetchTransactionHistory.fulfilled, (state, action) => {
                const pendingTxs = loadPendingTransactions();
                const pendingDeployIds = new Set(
                    pendingTxs.map((t) => t.deployId),
                );
                const confirmedDeployIds: string[] = [];

                const newTransactions = (action.payload as RChainTx[]).map(
                    (tx) => {
                        const isPendingInStorage = pendingDeployIds.has(
                            tx.deployId,
                        );
